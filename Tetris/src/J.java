...
public class J implements Tetrimino {
    ...
    public J() {
        $color = Color.BLUE;
        $points = new Point[]{
                new Point(Tetris.COLS/2-2, 0),
                new Point(Tetris.COLS/2-2, 1),
    ...
    public void spin() {
        ...
        for (int i = 0; i < 4; ++i) {
            $points[i].x = $points[2].x + $relPoints[i].x;
            $points[i].y = $points[2].y + $relPoints[i].y;
        }
